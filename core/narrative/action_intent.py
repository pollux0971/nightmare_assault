...
# 注意：**不放裸「研究」**——會誤中「研究站/研究員/研究室」等主題地名/職稱（見 _has_research_verb）。
_TRUTH = ("分析", "解讀", "破解", "推敲", "研判", "驗屍", "解剖", "比對",
          "實驗紀錄", "實驗記錄", "研究紀錄", "研究記錄", "異常頻率", "異常數據",
          "數據", "檔案", "日誌", "錄音", "病歷", "監控紀錄", "追查真相", "查清真相",
          "弄清真相", "查明真相", "挖出真相", "真相是什麼", "到底發生什麼", "深入研究",
          "解開謎", "破解謎", "搞清楚發生", "查清楚到底")
# 「研究」當動詞（調查）才算 truth；研究後接 站/員/室/所/中心/院/部/設(備) 等 → 是地名/物名，不算
_RESEARCH_FACILITY_SUFFIX = ("站", "員", "室", "所", "中心", "院", "部", "團", "隊", "設", "區", "者")


def _has_research_verb(t: str) -> bool:
    for m in _re.finditer("研究", t or ""):
        if t[m.end():m.end() + 1] not in _RESEARCH_FACILITY_SUFFIX:
            return True
    return False
...
_NO_TRUTH = ("不碰真相", "不追真相", "不查真相", "不想知道真相", "不深入", "不再深入",
             ...
             # 逃避型：不想管 / 只想離開（UX #4：no_truth 漏判「不想管 / 只想離開」）
             "不想管", "不想理", "不想追究", "不在乎真相", "不關心真相", "懶得管", "不想搞清楚",
             "只想離開", "只想出去", "只想找出口", "只想逃", "只想走", "只想離開這裡",
             "只要離開", "只要出去", "只要找到出口", "只想趕快離開", "只想趕快出去")
...
def classify_action(text: str, *, exploration_mode: str = "active_exploration",
    ...
    if not nt and (any(p in t for p in _TRUTH) or _has_research_verb(t)):
