...
### 補丁十四 · Opening Variation Contract（開場變體契約，v0.8）— 見 `19-opening-variation.md`

- **問題**：prompt 裡的具體名詞被 LLM 照抄——`紙條` 幾乎每局出現、`林晨` 反覆出現、開場目標幾乎都是 `找人`。根因不是文筆，是 prompt examples 形成高權重錨點。**不能用更多「不要重複」的 prompt 修；要把具體例子降級成抽象欄位 + 變體池。**
- **解法**：開局產生 `OpeningVariationContract`——`抽象 archetype 變體池 + 權重抽樣 + cooldown ledger + contract enforcement`。Setup/OpeningDirector 決定開場核心素材（motive_archetype / personal_anchor_type / message_medium / first_interactable_type），StoryAgent **只能執行、不可重抽**；LLM 仍負責把素材寫自然。
- **元件**：`opening_variation.py`（enums + Contract + CooldownLedger + weighted_choice）、`opening_pools.py`（降權過度重複素材 + medium→literals）、`opening_cooldown_store.py`（additive SQLite 跨 run 持久化 + run 計數）、`opening_variation_selector.py`（池+cooldown→契約，決定性 seed）、`opening_variation_gate.py`（forbidden literal/archetype + medium mismatch 偵測）、`opening_variation_prompt.py`（ctx 注入 + repair 指令 + 決定性 fallback）。
- **enforcement**：story 開場 output 含 forbidden literal / 偷換 medium → **repair 一次** → 仍違規用**決定性 fallback opening**（保證乾淨）；違規寫進 observation。
- **flag**：`ENABLE_OPENING_VARIATION_CONTRACT`（預設 OFF，獨立於 ENABLE_NARRATIVE_CONTROL）；OFF 時開場流程與補丁前完全一致。
- **邊界（非目標）**：不改 WorldModel / TruthEvidenceGate / SpatialProjection / PlayerState，不新增固定故事內容，不做模板引擎，**不強制收束劇情**（契約只決定開場素材，不決定走向/結局）。
- **狀態**：28 條測試（schema/cooldown/selector/gate/prompt/整合 flag ON repair·fallback）；**全測 920 passed，flag OFF/ON 各一次**。

> **里程碑（敘事多樣性）**：開場核心素材由抽象池決定、cooldown 擋近期重複、契約強制執行——`紙條/林晨/找人` 不再是預設答案。

### 補丁十五～十八 · 開放式探索 runtime 補強鏈（→ `20-open-exploration-rc.md`）

承補丁十四之後，依**真 LLM Full UX Selfplay** 暴露的缺口逐一補強（每項皆 observation/旁路、不改 gate、不進 story 生成）：

- **補丁十五 · Reveal Reward Loop**：gate=True 的 `truth_investigation` 常停 hinted、真相型無回報 → 對已 hinted/observed
  真相施 reward（hinted→observed→suspected）；**reward 上限 suspected、不直達 confirmed**；每拍記 `reveal_reward_debug`
  （gate_allowed / candidates / ladder_action / prev→next / no_progress_reason）。不碰 TruthEvidenceGate。
- **補丁十六 · PlayerState Payoff + Fragment Title**：take 動詞 + resolver→object → `taken`/inventory（inspect 不取；
  taken 退地面 visible，focus 例外）；reveal observed+ → **public-safe known_fact**（`public_title`：explicit 或 slug
  fallback，**永不「未命名的真相」、永不 hidden content**）；NPC fact→known_facts（label/source/confidence）。
- **補丁十七 · Spatial Routes + AliasResolver Focus-Scope + Actor 一致性**：`previous_area` + roles → 可走路線
  （return_previous/withdraw_safe/return_site/exit；不再「沒有明顯可走的出口」）；指代依 scope 解析
  （那個東西→object 不被 NPC focus 吃 / 他說的地方→fact / 那個人→actor）+ 強指代 anchor + bounded noun；
  `ensure_actor_entity` 讓 focus 一定對得到 WorldModel actor entity。
- **補丁十八 · 穩定化**：sanitizer 補繁簡 + 開場消毒 + placeholder/壞分隔符；WorldDelta `id` 漏網修；
  `no_truth_intent` 擋逃避型「不想管/只想離開」。

> **RC 里程碑**：以上整合為 **MVP-B Open Exploration Runtime = RC / stabilization candidate**——
> **mock 回歸 36/37**（`dev/tools/mock_full_ux_regression.py`，無真 LLM）、全測 1006 passed；唯一 monitor：
> `retreat → review_mode`（UX #6）；**目前不進 P5 story context**（待 GUI 驗證後）。詳見 `20-open-exploration-rc.md`。
